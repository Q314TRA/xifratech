...
const FeaturesArea = ({ featureStyle, anchorStyle, textStyle }) => {
    const featuresData = useStaticQuery(graphql`
        query ProcessingFeatureQuery {
            ...
            allItServicesJson(filter: {is_featured: {eq: true}}) {
                edges {
                    node {
                        title
                        fields {
    ...
    const services = featuresData.allItServicesJson.edges;

    return (
        <SectionWrap id="servicios">
            <Container>
                <Row>
                    <Col lg={12}>
                        <SectionTitle
                ...
                <Row>
                    <Col lg={12}>
                        <SectionBottom>
                            <Text {...textStyle}>
                                Guía el desarrollo de tu empresa. <Anchor {...anchorStyle} path="#contacto">Toma el reto!</Anchor>
                            </Text>
                        </SectionBottom>
                    </Col>
                </Row>
